...
class ModelController {
    ...
    static async handleLive2DModel(modelName, res) {
        ...
        // 直接拼接model目录，因为modelName现在不包含前导斜杠
        ...
        if (await fs.pathExists(modelPath)) {
            try {
                ...
                // 使用 ModelService 的路径处理方法，传入正确的基础路径
                const basePath = `./${relativeModelDir}`;
                const processedConfig = ModelService.processModelPaths(modelConfig, basePath);
    ...
    static async handleStandardModel(modelName, res) {
        ...
        console.log(`[DEBUG] /get - 标准模型配置文件路径: ${indexPath}`);

        if (!(await fs.pathExists(indexPath))) {
            console.log(`[DEBUG] /get - 标准模型配置文件不存在`);
    ...
    static async scanModels(req, res) {
        try {
            ...
            const { scanModels } = require('../scripts/scanModels');
            await scanModels();
        ...
        try {
            ...
            if (PathUtils.isNetworkUrl(modelName)) {
                ...
                if (await fs.pathExists(modelPath)) {
                    ...
                    const modelDir = path.dirname(modelPath);
                    const relativeModelDir = path.relative(path.join(__dirname, '..'), modelDir).replace(/\\/g, '/');
                    modelConfig = ModelService.processModelPaths(modelConfig, `./${relativeModelDir}`);
                } else {
                    return res.status(404).json({ error: '模型文件不存在' });
                }
            } else {
                const indexPath = path.join(__dirname, '..', 'model', modelName, 'index.json');
                if (await fs.pathExists(indexPath)) {
                    modelConfig = await fs.readJson(indexPath);
                    modelConfig = ModelService.processModelPaths(modelConfig, `./model/${modelName}`);
                } else {
                    return res.status(404).json({ error: '模型配置文件不存在' });
                }
            }
    ...
    static async getModelConfig(req, res) {
        try {
            const { modelName } = req.params;
            console.log(`[DEBUG] /model/${modelName}/index.json - 请求模型配置`);
            
            // 检查是否为网络模型
            if (PathUtils.isNetworkUrl(modelName)) {
                console.log(`[DEBUG] /model/${modelName}/index.json - 网络模型不支持此端点`);
            ...
            if (PathUtils.isLive2DModelPath(modelName)) {
                ...
                if (await fs.pathExists(modelPath)) {
                    ...
                    const modelDir = path.dirname(modelPath);
                    const relativeModelDir = path.relative(path.join(__dirname, '..'), modelDir).replace(/\\/g, '/');
                    const processedConfig = ModelService.processModelPaths(modelConfig, `./${relativeModelDir}`);
                    console.log(`[DEBUG] /model/${modelName}/index.json - 返回 live2d-model-assets 模型配置`);
                    return res.json(processedConfig);
                } else {
                    console.log(`[DEBUG] /model/${modelName}/index.json - live2d-model-assets 模型文件不存在`);
                    return res.status(404).json({ error: '模型文件不存在' });
                }
            }
            ...
            if (!(await fs.pathExists(indexPath))) {
                console.log(`[DEBUG] /model/${modelName}/index.json - 标准模型配置文件不存在`);
                return res.status(404).json({ error: '模型配置文件不存在' });
            }
            ...
            console.log(`[DEBUG] /model/${modelName}/index.json - 返回标准模型配置`);
            res.json(processedConfig);
